...
# RecSys
class RecSysNetwork(torch.nn.Module):
...
class SingleSourceRecSysNetwork(RecSysNetwork):
    ...
    def __init__(self, modality_features, dropout_value):
        ...
        self.allowed_dims = [4096, 2048, 1024, 768, 512, 384, 192, 128, 100, 64]
        ...
        if starting_dim not in self.allowed_dims:
            raise ValueError(f'Embedding dimensionality not supported: passed {starting_dim},\n'
                f' possible dims:{set(self.allowed_dims)}')
...
class RecSysNetworkBasic(SingleSourceRecSysNetwork):
...
class RecSysNetworkBasicAllItems(SingleSourceRecSysNetwork):
