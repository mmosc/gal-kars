...
if __name__ == "__main__":
    ...
    train_ratings = pd.read_table(os.path.join(RAW_DATA_DIR, 'lfm-2b', 'train.tsv'),
                                  names = ['user', 'item', 'score'], sep='\t')
    print(f'New train len {len(train_ratings)}')

    test_ratings = pd.read_table(os.path.join(RAW_DATA_DIR, 'lfm-2b', 'test.tsv'),
                                  names = ['user', 'item', 'score'], sep='\t')
    ...
    with open(f'/home/marta/jku/gal-kars/GAL-KARS/2_learn_embs/lfm-2b/top50_users_to_unique_id.pkl', 'rb') as f:
        top50_users_to_unique_id = pkl.load(f)
    ...
    # Convert artists to sequential ids
    with open(f'/home/marta/jku/gal-kars/GAL-KARS/2_learn_embs/lfm-2b/top50_artist_to_unique_id.pkl', 'rb') as f:
        top50_artist_to_unique_id = pkl.load(f)

    emb_folders = ['../data/embeddings/lfm-2b/']
    ...
    source_features = dict(sorted((pkl.load(open('../data/embeddings/lfm-2b/lfm-2b_s=1_CompGCN_k=64_l=1.pkl', 'rb'))).items()))
    ...
    feature_list = [ 
                    ['lfm-2b_s=1_CompGCN_k=64_l=1'], ['lfm-2b_s=1_CompGCN_k=64_l=2'], ['lfm-2b_s=1_CompGCN_k=64_l=3'],
                    ['lfm-2b_s=2_CompGCN_k=64_l=1'], ['lfm-2b_s=2_CompGCN_k=64_l=2'], ['lfm-2b_s=2_CompGCN_k=64_l=3'],
                    ['lfm-2b_s=3_CompGCN_k=64_l=1'], ['lfm-2b_s=3_CompGCN_k=64_l=2'], ['lfm-2b_s=3_CompGCN_k=64_l=3'],
        ]
    ...
    for i, config in enumerate(feature_list):
            ...
            preds_folder = f'reports/lfm-2b/predictions/{setting_sources[len(x) - 1]}/'
            model_folder = f'reports/lfm-2b/models/{setting_sources[len(x) - 1]}/'
